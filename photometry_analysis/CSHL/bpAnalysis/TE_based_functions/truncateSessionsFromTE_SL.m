...
function TE = truncateSessionsFromTE_SL(TE, action, lickField, rewardTrials)
...
% rewardTrials- trials with reward, defaults to trialOutcome = 1.
    ...
    switch action
        case 'init'
            ...
            % plot reward collection latency --- SL
            yyaxis right
            TRUNC.latency = smooth(TE.latency(TRUNC.rewardTrials), 3);
%             TRUNC2.truncTrialHandle = zeros(1, nSessions); % will contain handles for TRUNC2 trial indicators               
            plot(TRUNC.rewardTrials, TRUNC.latency); 
            ylabel('RewardCollectLatency'); set(gca, 'YLim', [0 1]);
            hold on;  
            
            % plot reward period lick rate
            yyaxis left
            if isempty(lickField)
                TRUNC.rewardLicks = smooth(TE.usLicks.rate(TRUNC.rewardTrials), 'moving', 3);
            else
                TRUNC.rewardLicks = smooth(TE.(lickField).rate(TRUNC.rewardTrials), 'moving', 3);
            end

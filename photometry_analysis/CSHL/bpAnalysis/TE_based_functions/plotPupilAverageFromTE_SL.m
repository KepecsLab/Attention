function varargout = plotPupilAverageFromTE_SL(TE, trials, varargin)
...
% trials- may be cell array of trials for different conditions
    ...
    avgData = pupilAverageFromTE(TE, trials, varargin{:}); % compile averages 

    ax = s.ax;
    nLines = size(avgData.Avg, 1);
    xData = avgData.xData(1,:);    % fixed across trial subsets, see phAverageFromTE
    for counter = 1:nLines
%         avgData.Avg(counter, :) = smooth(avgData.Avg(counter, :), 5); % smooth_SL

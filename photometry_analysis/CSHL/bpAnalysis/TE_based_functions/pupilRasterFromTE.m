function ih = pupilRasterFromTE(TE, trials, varargin)
    ...
    defaults = {...
        ...
        'PhotometryField', 'pupil';...
        'FluorDataField', 'pupDiameterNorm';...
        };
...
%         s.zeroTimes = s.zeroTimes + Photometry.startTime; % convert to Bpod time
        ...
        if isempty(s.window)
            s.window = Pupil.xData([1 end]);
        end
    end
    varargin = rewrite_varargin(s, varargin);
    
    [cData, xData] = pupilAlignedWindow(TE, trials, varargin{:});
    ...
    % eye)
    if isempty(s.CLim)
        imavg = nanmean(nanmean(Pupil.(s.FluorDataField), 1), 2);
        imstd = nanmean(nanstd(Pupil.(s.FluorDataField), 1), 2);

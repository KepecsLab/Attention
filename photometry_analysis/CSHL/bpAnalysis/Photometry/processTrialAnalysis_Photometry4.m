function Photometry = processTrialAnalysis_Photometry4(sessions, varargin)
% exemplar for new trial analysis functions
    ...
    defaults = {...
        ...
        'blMode', 'byTrial';... % 'byTrial', 'bySession', 'expFit', 'median'  expFit- interpolates baseline from biexponential fit to raw fl baselines across trials
        ...
        'expFitBegin', 0.1;...
        'expFitEnd', 3.5;...% make a different expFit window from (0.1 1) ---SL 7/20/2023
        'zeroField', 'Us';...
...
%         free parameter
    ...
    if ~iscell(s.expFitEnd)  
        s.expFitEnd = repmat({s.expFitEnd}, 1, max(s.channels)); % make a different expFit window ---SL 7/20/2023
    end
...
%% Initialize
    ...
    if s.uniformOutput % different sized trials are padded with NaNs, aligned to photometry start
        data = struct(...
            ...
            'BLdF', NaN(totalTrials, 1),... % mean deltaF during baseline period 
            'ch', []...
            );    
...
%                     trialFit = trialFit - nanmean(trialFit(1, expFitStartP:blEndP));
            ...
            if s.uniformOutput
                ...
                Photometry.data(fCh).BLdF(tcounter:tcounter+nTrials - 1, 1) = mean(dF(:,blStartP:blEndP), 2);
            else
                dFF = dF ./ blF;
                sd = nanmean(nanstd(dF(:,blStartP:blEndP), 0, 2));
                ZS = dF / sd;

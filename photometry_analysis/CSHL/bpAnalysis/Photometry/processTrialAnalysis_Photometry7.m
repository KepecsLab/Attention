function Photometry = processTrialAnalysis_Photometry7(sessions, varargin)
...
%% Initialize
    ...
    if s.uniformOutput % different sized trials are padded with NaNs, aligned to photometry start
        data = struct(...
            ...
            'ddF', NaN(totalTrials, expectedSamples),...
            'ddFZS', NaN(totalTrials, expectedSamples),...
            'fdF', NaN(totalTrials, expectedSamples),...
            'fRaw', NaN(totalTrials, expectedSamples),...
            'Fcorr', NaN(totalTrials, expectedSamples),...
            'ratioF', NaN(totalTrials, expectedSamples),...
            'ch', []...
            );    
    ...
    for si = 1:length(sessions)
        ...
        if ~isfield(SessionData, 'demod')
            SessionData = demodulateSession_SL(SessionData, 'channels', s.channels, 'refChannels', s.refChannels, 'forceAmp', s.forceAmp, 'ACfilter', s.ACfilter); % don't necessarily want to save these back to sessions because that'd eat up memory
        end
...
%             blF_raw = NaN(nTrials, 1);
            ...
            for trial = 1:nTrials
                ...
                expFitEndP = min(bpX2pnt(s.expFitEnd{chCounter}, sampleRate), downSamples); % make a different expFit window ---SL 7/20/2023
            ...
            % always perform session bleaching fit and store it for later use

%             blF_fit = nanmean(allData(:, expFitStartP:blEndP), 2); % take mean across time, not trials     
            blF_fit = nanmean(allData(:, expFitStartP:expFitEndP), 2); % make a different expFit window ---SL 7/20/2023             
...
%                     blF_raw = nanmean(allData(:, blStartP:blEndP), 2);
                    blF_raw = nanmean(allData(:, expFitStartP:expFitEndP), 2);
                case 'median' % median filtering in the preCSrecording window for +-3 trials  ---SL         
                    blF_raw = nanmean(allData(:, blStartP:blEndP), 2);
                    blF = medfilt1(blF_raw, 3, 'omitnan', 'truncate');  
%                     blF = medfilt1(blF_raw, 7, 'omitnan', 'truncate'); 
%                     median_fit = medfilt1(blF_raw, 7, 'omitnan', 'truncate'); 
%                     blF = repmat(median_fit, 1, size(allData, 2)); % moving median is a vector
                otherwise
            end
...
%                     trialFit = trialFit - nanmean(trialFit(1, expFitStartP:blEndP));
                    trialFit = trialFit - nanmean(trialFit(1, expFitStartP:expFitEndP));
                    blF = bsxfun(@plus, blF, trialFit);
                    dF = allData - blF;
                otherwise
            end
            if s.uniformOutput
                Fcorr = dF + nanmean(nanmean(allData(:, blStartP:blEndP), 2)); % SCALAR mean across time and trials;
                Photometry.data(fCh).Fcorr(tcounter:tcounter+nTrials - 1, :) = Fcorr; 
                ...
                Fcorr = dF + nanmean(nanmean(allData(:, blStartP:blEndP), 2)); % SCALAR mean across time and trials;
                for trial = 1:nTrials
                    Photometry.data(fCh).Fcorr{tcounter + trial - 1} = Fcorr(trial, 1:sampleCount(trial)); 
        ...
        % add movement correction by subtracting or dividing ch1 by ch2
        if s.uniformOutput
            ratioF = Photometry.data(1).Fcorr(tcounter:tcounter+nTrials - 1, :) ./ Photometry.data(2).Fcorr(tcounter:tcounter+nTrials - 1, :);  
            ddF = Photometry.data(1).dF(tcounter:tcounter+nTrials - 1, :) - Photometry.data(2).dF(tcounter:tcounter+nTrials - 1, :);  
            fdF = Photometry.data(1).dF(tcounter:tcounter+nTrials - 1, :) ./ Photometry.data(2).dF(tcounter:tcounter+nTrials - 1, :);
            fRaw = Photometry.data(1).raw(tcounter:tcounter+nTrials - 1, :) ./ Photometry.data(2).raw(tcounter:tcounter+nTrials - 1, :);  
            Photometry.data(1).ratioF(tcounter:tcounter+nTrials - 1, :) = ratioF; 
            Photometry.data(1).ddF(tcounter:tcounter+nTrials - 1, :) = ddF; 
            Photometry.data(1).fdF(tcounter:tcounter+nTrials - 1, :) = fdF; 
            Photometry.data(1).fRaw(tcounter:tcounter+nTrials - 1, :) = fRaw;
        else
            ratioF = Photometry.data(1).Fcorr(tcounter:tcounter+nTrials - 1, :) ./ Photometry.data(2).Fcorr(tcounter:tcounter+nTrials - 1, :);  
            ddF = Photometry.data(1).dF(tcounter:tcounter+nTrials - 1, :) - Photometry.data(2).dF(tcounter:tcounter+nTrials - 1, :);
            fdF = Photometry.data(1).dF(tcounter:tcounter+nTrials - 1, :) ./ Photometry.data(2).dF(tcounter:tcounter+nTrials - 1, :);
    ...
    %             blStartP = min(bpX2pnt(s.baseline{chCounter}(1), sampleRate), downSamples);
    %             blEndP = min(bpX2pnt(s.baseline{chCounter}(2), sampleRate), downSamples); 
                Photometry.data(1).ratioF{tcounter + trial - 1} = ratioF(trial, 1:sampleCount(trial));            
                Photometry.data(1).ddF{tcounter + trial - 1} = ddF(trial, 1:sampleCount(trial));
                Photometry.data(1).fdF{tcounter + trial - 1} = fdF(trial, 1:sampleCount(trial));      
                Photometry.data(1).fRaw{tcounter + trial - 1} = fRaw(trial, 1:sampleCount(trial));  
            end
        end 
        
        if s.uniformOutput
            ddF_sd = nanmean(nanstd(ddF(:,blStartP:blEndP), 0, 2));
            Photometry.data(1).ddFZS(tcounter:tcounter+nTrials - 1, :) = ddF / ddF_sd; % z-scored
        else
            ddF_sd = nanmean(nanstd(ddF(:,blStartP:blEndP), 0, 2));
            ddFZS = ddF / ddF_sd;
            for trial = 1:nTrials
                Photometry.data(1).ddFZS{tcounter + trial - 1} = ddFZS(trial, 1:sampleCount(trial));
            end
        end 

...
%             blF_raw = NaN(nTrials, 1);
            ...
            blF_fit = nanmean(allData(:, expFitStartP:blEndP), 2); % take mean across time, not trials, 1 value for each trial   
%             blF_fit = nanmean(allData(:, 3:81), 2); % take mean across time, not trials. Fixed window for b1F_fit as[0.1 4];
...
%             warning('check this in processTrialAnalysis_Photometry2, x should start at 0 for session bleach fit'); % old --> new
            ...
            switch blMode
                case 'byTrial'
                    blF = nanmean(allData(:, blStartP:blEndP), 2); % take mean across time, not trials, 1 value for each trial  
                    blF_raw = blF;                    
...
%                     blF_raw = nanmean(allData(:, expFitStartP:blEndP), 2);
                    blF_raw = nanmean(allData(:, blStartP:blEndP), 2);
                    blF = medfilt1(blF_raw, 3, 'omitnan', 'truncate'); % moving median is a single value?  
...
%%
            switch dFFMode
                case 'simple'
                    dF = allData - blF;
                case 'expFit'
%                     trialMeanY = (nanmean(allData(:, expFitStartP:blEndP), 1))'; % take mean across trial, not time, 1 value for mean of each 0.05s  
%                     trialMeanX = ((0:length(trialMeanY) - 1)/sampleRate + expFitStartP/sampleRate)';  
                    trialMeanY = (nanmean(allData(:, 3:81), 1))'; % fixed window for in-trial expFit as [0.1 4]; ---SL
                    trialMeanX = ((0:length(trialMeanY) - 1)/sampleRate + 3/sampleRate)';  
%                     trialMeanYFull = (nanmean(allData(:, 1:blEndP), 1))';      
                    trialMeanYFull = (nanmean(allData(:, 1:81), 1))';  % fixed window for in-trial expFit as [0.1 4]; ---SL
...
%                     trialFit = trialFit - nanmean(trialFit(1, expFitStartP:blEndP));
                    trialFit = trialFit - nanmean(trialFit(1, 3:81));  % fixed window for in-trial expFit as [0.1 4]; ---SL

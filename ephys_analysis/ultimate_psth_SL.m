function [psth spsth spsth_se tags spt stats] = ultimate_psth_SL(cellid,event_type,event,window,varargin)
...
% Events, time, valid trials, SL_20250903 
...
if iscell(event)   % different ref. event for tst and baseline window
    ...
    NumTrials = length(VE.TrialStart);   % number of trials
    fr_stim_01_0 = nan(NumTrials-1,1);
    for iT = 1:NumTrials-1     % last trial may be incomplete   
        ...
        lspikes = stimes{iT};   % spikes relative to the event
        ...
        rts(iT) = VE.ReactionTime(iT);   
    end
    threshold = find(fr_stim_01_0<prctile(fr_stim_01_0, 98));  % exclude trials with top 2% highest firing rate     
%     rts = VE.ReactionTime;
%     rts = rts(~isnan(rts));   % all RT values
%     rtlow = 0;   % all lick trials RT ranking
%     rthigh = 100;   % all lick trials RT ranking
%     threshold = find(VE.ReactionTime>prctile(rts,rtlow) & VE.ReactionTime<=prctile(rts,rthigh));  % exclude trials with top 2% highest firing rate
%     valid = filterTrials_SL(cellid,'event_type',event_type,'event',event,...
%         'event_filter',g.event_filter,'filterinput',g.filterinput);
    valid_trials = intersect(valid, threshold);
end

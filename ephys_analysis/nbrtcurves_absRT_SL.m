function nbrtcurves_absRT_SL(I,issave)
...
doraster = false;
align = 'tone';  % controls alignment: 'tone' or 'response' or 'feedback'
normalization = 'BLzscore';  % controls normalization: 'zscore' or 'max' or 'BLzscore'

% Directories
% global DATAPATH
% fs = filesep;
% resdir = [DATAPATH 'NB' fs 'rtcurves_new_' align '_' group fs];   % results directory
resdir = ['C:\Kepecs lab\PV project\Balazs data\NBPV\' whichcb '_' align '_RT\']; 
% PSTH
% trial_type = 'hit';
trial_type = 'fa';
poppsth(I,resdir,align,trial_type,normalization,doraster,issave); %#ok<*ASGLU>
...
blwn = [-4 0];  % SL_20250903
bltime = blwn(1):dt:blwn(2);   % time vector
bllength = length(bltime);
blstart =  (blwn(1)-wn(1))/dt +1; % SL_20250903
blend =  (blwn(end)-wn(1))/dt;

sigm = 0.05;  % SL_20250903
wn2 = [-0.5 0];   % in seconds
time2 = wn2(1):dt:wn2(2);  % time vector
wn2start =  (wn2(1)-wn(1))/dt +1;  % SL_20250903
wn2end =  (wn2(end)-wn(1))/dt;
...
allpsth0_orig = [];  % all PSTHs, for all types
allspsth0_orig = []; % all smoothed PSTHs, for all types
NumCell = length(I);  % number of cells

% create pooled structure, SL_20250903       
t2 = struct(...             
    'data', cell(1,1),...
    'avg', nan(NumCell, 1),... 
    'sem', nan(NumCell, 1)...
...
%     hitfilter = 'selectGoRTabs'; %#ok<NASGU>
%     fafilter = 'selectNoGoRTabs'; %#ok<NASGU>
    ...
    try        
        ...
        [psth1, spsth1, spsth_se1, ~, spt1] = ultimate_psth(cellid,'trial',cevent,wn,...
            'dt',dt,'display',true,'sigma',sigm,'parts','all',...
            'isadaptive',2,...
        ...
        [psth4, spsth4, spsth_se4, ~, spt4] = ultimate_psth(cellid,'trial',cevent,wn,...
            ...
            'maxtrialno',Inf,'baselinewin',[-0.5 0],'testwin',[0 0.5],...
            'relative_threshold',0.1);
        [psth0, spsth0, spsth_se0, ~, spt0] = ultimate_psth(cellid,'trial',cevent,wn,...
            'dt',dt,'display',true,'sigma',sigm,'parts','all',...
            'isadaptive',2,...
            'event_filter','lick',...
            'maxtrialno',Inf,'baselinewin',[-0.5 0],'testwin',[0 0.5],...
            'relative_threshold',0.1);
        [psthto, spsthto, spsth_seto, ~, sptto] = ultimate_psth(cellid,'trial',cevent,wn,...
            'dt',dt,'display',true,'sigma',sigm,'parts','all',...
            'isadaptive',2,...
            'event_filter',cfilter,'filterinput',[0 1],...
            'maxtrialno',Inf,'baselinewin',[-0.5 0],'testwin',[0 0.5],...
            'relative_threshold',0.1);
        ...
        disp(size(spt0,1))  % display number of trials 
        ...
        allpsth0_orig(1,linx,1:length(time)) = psth0; %#ok<AGROW>
        allspsth0_orig(1,linx,1:length(time)) = spsth0; %#ok<AGROW>
        allspsth_se0_orig(1,linx,1:length(time)) = spsth_se0; %#ok<AGROW>
        % save data into pooled structure, SL_20250903  
        ordering = {...
            'RT1', 1;...
            'RT2', 2;...
            'RT3', 3;...
        ...
        switch trialtype
            case 'hit'   
                for c2 = 1:size(ordering,1)    
                    thisdata = spsth(c2, wn2start:wn2end);
                    FR_pooled.hit.(ordering{c2,1}).data{k,:} = thisdata;
                    FR_pooled.hit.(ordering{c2,1}).avg(k,:) = nanmean(thisdata);
                    FR_pooled.hit.(ordering{c2,1}).sem(k,:) = nanmean(spsth_se((ordering{c2,2}), wn2start:wn2end));
                end
            case 'fa'
                for c2 = 1:size(ordering,1)    
                    thisdata = spsth(c2, wn2start:wn2end);
                    FR_pooled.fa.(ordering{c2,1}).data{k,:} = thisdata;
                    FR_pooled.fa.(ordering{c2,1}).avg(k,:) = nanmean(thisdata);
                    FR_pooled.fa.(ordering{c2,1}).sem(k,:) = nanmean(spsth_se((ordering{c2,2}), wn2start:wn2end));
                end
        end           
        
        % Plot PSTH for individual cell
        figure
        hold on
        ...
        if issave
            ...
            fnm = [resdir cellidt '_PSTH_absRTquartile_' trialtype '.fig'];
            saveas(H,fnm)
        end
%         close(H)
...
%             ./ repmat(std(mean(allspsth_orig,1),[],3),[lent,1,lenp]);  % standardize based on mean across conditions
        allpsth = (allpsth_orig - repmat(mean(allpsth0_orig, 3),[lent,1,lenp])) ...
            ./ repmat(std(allpsth0_orig, [], 3),[lent,1,lenp]);   % standardize based on mean across conditions, SL_20250903
        allspsth = (allspsth_orig - repmat(mean(allspsth0_orig, 3),[lent,1,lenp])) ...
            ./ repmat(std(allspsth0_orig, [], 3),[lent,1,lenp]);   % standardize based on mean across conditions, SL_20250903

    case 'BLzscore'
        allspsth0_orig_BL = allspsth0_orig(1, :, blstart:blend);
        allspsth_se0_orig_BL = allspsth_se0_orig(1, :, blstart:blend);
        allpsth = (allpsth_orig - repmat(mean(allspsth0_orig_BL, 3),[lent,1,lenp])) ...
            ./ repmat(mean(allspsth_se0_orig, 3),[lent,1,lenp]);   % standardize based on deviation across trials, SL_20250903
        allspsth = (allspsth_orig - repmat(mean(allspsth0_orig_BL, 3),[lent,1,lenp])) ...
            ./ repmat(mean(allspsth_se0_orig, 3),[lent,1,lenp]);   % standardize based on deviation across trials, SL_20250903
%         allpsth = (allpsth_orig - repmat(mean(allspsth0_orig_BL, 3),[lent,1,lenp])) ...
%             ./ repmat(std(allpsth0_orig, [], 3),[lent,1,lenp]);   % standardize based on deviation across time
%         allspsth = (allspsth_orig - repmat(mean(allspsth0_orig_BL, 3),[lent,1,lenp])) ...
%             ./ repmat(std(allspsth0_orig, [], 3),[lent,1,lenp]);   % standardize based on deviation across time
    case 'max'
        allpsth = (allpsth_orig - repmat(mean(mean(allpsth_orig,1),3),[lent,1,lenp])) ...
...
H = gcf;
if issave
    fnm = [resdir 'meanPSTH_absRTquartile_' trialtype '.fig'];
    saveas(H,fnm)
end
% close(H)
...
H = gcf;
if issave
    fnm = [resdir 'meanSPSTH_absRTquartile_' trialtype '.fig'];
    saveas(H,fnm)
end
% close(H)

save(fullfile(resdir, 'allspsth_orig.mat'), 'allpsth_orig', 'allspsth_orig', 'allpsth0_orig', 'allspsth0_orig', 'allspsth_se0_orig', 'allspsth');
save(fullfile(resdir, 'FR_pooled.mat'), 'FR_pooled');

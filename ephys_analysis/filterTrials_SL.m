function valid_trials = filterTrials_SL(cellid,varargin)
...
switch event_filter
    ...
    case 'selectGoRT'
        ...
        rtthres = prctile(rts,prcthres*100);   % all lick trials RT ranking
    ...
    case 'selectNoGoRTLick'
        ...
        valid_trials = find(VE.NoGoRT>rtthres(1)&VE.NoGoRT<=rtthres(2));   % RTs of the percentile interval
    case 'selectGoRTabs'
        rtthres = filterinput;   % percentile threshold
        valid_trials = find(VE.GoRT>rtthres(1)&VE.GoRT<=rtthres(2));   % absolute RTs SL_20250903
    case 'selectNoGoRTabs'
        rtthres = filterinput;   % percentile threshold
        valid_trials = find(VE.NoGoRT>rtthres(1)&VE.NoGoRT<=rtthres(2));   % absolute RTs  SL_20250903    
    case 'none'
        valid_trials = find(~isnan(VE.(event)));
    case 'custom'
        valid_trials = selecttrial(VE,filterinput);
    case 'lick'
        valid_trials = find(~isnan(VE.ReactionTime));   % both hit and FA SL_20250903
%     case 'lick_abs'
%         rtthres = filterinput;   % percentile threshold
%         valid_trials = find(~isnan(VE.ReactionTime) & VE.GoRT>=rtthres(1)&VE.GoRT<=rtthres(2));   % RTs of the percentile interval
%     case 'lick_prctile'
%         prcthres = filterinput;   % percentile threshold
%         rts = VE.GoRT;
%         rts = rts(~isnan(rts));   % all RT values

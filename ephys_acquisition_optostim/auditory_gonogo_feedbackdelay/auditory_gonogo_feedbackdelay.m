function [obj] = auditory_gonogo_feedbackdelay(varargin)
...
switch action
    ...
    case 'init'
        ...
        NumeditParam(obj, 'IdleTrials2Suspend', 10000, xpos, ypos); next_row(ypos,1.25);
        NumeditParam(obj, 'Punish_ITI', 1, xpos, ypos); next_row(ypos,1.25);
        NumeditParam(obj, 'FeedbackDelay',[0.2 0.4 0.3 0.03],xpos, ypos,'TooltipString','min,max,mean,sd'); next_row(ypos,1.25);
        NumeditParam(obj, 'DrinkTime', 2, xpos, ypos); next_row(ypos,1.25);
        NumeditParam(obj, 'RespDur',4, xpos, ypos); next_row(ypos,1.25);
        NumeditParam(obj, 'Delay2Resp',0.8, xpos, ypos); next_row(ypos,1.25);
        NumeditParam(obj, 'StimDur', [0.5 0.1 0.05], xpos, ypos); next_row(ypos,1.25);
        NumeditParam(obj, 'StimProb', .5, xpos, ypos); next_row(ypos,1.5);
        
        % ------------- OPTOGENETICS PARAMETERS ---------------------------
        NumeditParam(obj, 'LightProb', 0, xpos, ypos); next_row(ypos,1.25);
        NumeditParam(obj, 'LightProb2', 0, xpos, ypos); next_row(ypos,1.25);
        NumeditParam(obj, 'LightDur', 1, xpos, ypos); next_row(ypos,1.25);
        NumeditParam(obj, 'LightLatency', 0.5, xpos, ypos); next_row(ypos,1.25);
        ...
        next_row(ypos, 1.5);
        xpos = 440; ypos=5;
        NumeditParam(obj, 'ITIMax', 15, xpos, ypos); next_row(ypos,1.25);
        NumeditParam(obj, 'ITIMin', 2, xpos, ypos); next_row(ypos,1.25);
        NumeditParam(obj, 'ITISD',3, xpos, ypos); next_row(ypos,1.25);
        NumeditParam(obj, 'ITIMean', 6, xpos, ypos); next_row(ypos,1.25);
        MenuParam(obj, 'ITIDistrib', {'EXP', 'UNIFORM', 'GAUSS', 'BIMODAL', 'UNIMODAL'}, 'GAUSS', xpos, ypos); next_row(ypos);
        SubheaderParam(obj, 'title', 'ITI Distribution parameters', xpos, ypos); next_row(ypos, 1.25);
        ...
        SoloParamHandle(obj, 'FeedbackDelayList', 'value', zeros(1,MaxTrials));
...
%         end
        ...
        set_callback({SignalModality; NumBlocks; TrialsPerBlock; StimProb; StimDur; StimDurList;LightDur;LightProb},{'auditory_gonogo_feedbackdelay','Update_TrialTypes'});
        set_callback({ITIMean; ITISD; ITIMin; ITIMax; ITIDistrib},{'auditory_gonogo_feedbackdelay','Update_TrialTypes'});
        set_callback({SoundDuration; SoundFrequency; SoundIntensity},{'auditory_gonogo_feedbackdelay','Update_TrialTypes'});
%         set_callback({SoundDuration; SoundFrequency; SoundIntensity},{'MakeAndUploadSounds','update'});
        ...
        auditory_gonogo_feedbackdelay(obj,'Update_TrialTypes');
...
%         hist(ITIs,30)
        ...
        mn2 = mns - mn;
        mx2 = mx - mn;
        temp = normrnd(mn2,sdg,NumTrials,1);
        while any(temp>mx2) || any(temp<0)
            inx = temp > mx2 | temp < 0;
            temp(inx) = normrnd(mn2,sdg,sum(inx),1);
        end
        temp = temp + mn;
        FeedbackDelayList(FutureTrialIndices) = temp;
        
        SoloFunctionAddVars('StateMatrixSection', 'rw_args',...
            {'TrialTypes','TrialTypeList','ITIs','FeedbackDelayList','StimDurList','LightStimList'});

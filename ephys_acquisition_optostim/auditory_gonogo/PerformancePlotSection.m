...
%%%  $Revision: 1.1 $
%%%  $Date: 2008-05-02 01:23:44 $
%%%  $Source: /cvs/ExperPort/Protocols/@saja_detection/PerformancePlotSection.m,v $
%%%  modified by SPR for mice_susattn

function [xpos, ypos] = PerformancePlotSection(obj, action, varargin)
...
switch action
  case 'init',
    ...
    SoloParamHandle(obj, 'validdot', 'saveable', 0, 'value',...
                    plot(-1, 1, 's','Color', [.75 .75 .75],'MarkerSize',MarkerSize));hold on;
    ...
    legend([value(type1dot),value(type2dot)],LegendStrings,...
    ...
    SoloParamHandle(obj, 'WindowSize', 'type', 'edit', 'label', 'WindowSize', ...
                    ...
                    'value', 20, 'position', round([MyFigPosition(3:4).*[0.92,0.64], 35, 40]));
    ...
    SoloParamHandle(obj, 'ValidVec', 'value', NaN(MaxTrialsLocal,1));
  ...
  case 'update'
    ...
    LastTrialValidCount = sum(HitHistoryLocal(1:LastTrial)>=0);
    TrialsType = TrialsType(HitHistoryLocal>=0);
    HitHistoryValid = HitHistoryLocal(HitHistoryLocal>=0);
    
    [mn, mx] = PerformancePlotSection(obj,'update_xlim',LastTrial);
    if(LastTrial>0)
        % -- Calculate fraction of valid trials --
        TrialsToInclude = [max(1,LastTrial-value(WindowSize)+1):LastTrial];
        ValidVec(LastTrial) = mean(HitHistoryLocal(TrialsToInclude)>=0);
        set(value(validdot), 'XData', [mn:mx], 'YData', ValidVec([mn:mx]));
        
        % -- Calculate total performance --
        TrialsToInclude = [max(1,LastTrialValidCount-value(WindowSize)+1):LastTrialValidCount];
        PerformanceVec(LastTrial) = mean(HitHistoryValid(TrialsToInclude)>0);
        set(value(kdot), 'XData', [mn:mx], 'YData', PerformanceVec([mn:mx]));

        % -- Calculate performance for each trial type --
        if(~isempty(TrialsType))
            TheseType1Trials = TrialsToInclude(TrialsType(TrialsToInclude)); %%% LEFT
            %%% Added "IF" to fix bug: it will give warning DivByZero if mean of empty vec.
            if(~isempty(TheseType1Trials))
                PerfType1Vec(LastTrial) = mean(HitHistoryValid(TheseType1Trials)>0);
            end
            set(value(type1dot), 'XData', [mn:mx], 'YData', PerfType1Vec([mn:mx]));

            TheseType2Trials = TrialsToInclude(~TrialsType(TrialsToInclude)); %%% RIGHT
            %%% Added "IF" to fix bug: it will give warning DivByZero if mean of empty vec.
            if(~isempty(TheseType2Trials))
                PerfType2Vec(LastTrial) = mean(HitHistoryValid(TheseType2Trials)>0);
            end

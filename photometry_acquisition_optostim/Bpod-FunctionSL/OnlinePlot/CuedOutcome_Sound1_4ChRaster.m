function CuedOutcome_Sound1_4ChRaster(Op, varargin)
    ...
    switch Op
        case 'init' 
            ...
            BpodSystem.ProtocolFigures.phRaster.SoundsToPlot = ls.SoundsToPlot;
            ...
            if any(channelsOn == 3)
                BpodSystem.ProtocolFigures.phRaster.fig_ch3 = ensureFigure('phRaster_ch3', 1);        
                nAxes = numel(ls.SoundsToPlot);        
                % params.matpos defines position of axesmatrix [LEFT TOP WIDTH HEIGHT].    
                params.cellmargin = [0.02 0.02 0.02 0.02];   
                params.matpos = [0 0 0.2 1];
                ...
                hAx = horzcat(hAx, axesmatrix(1, nAxes, 1:nAxes, params, gcf));  
                set(hAx, 'NextPlot', 'Add');
                BpodSystem.ProtocolFigures.phRaster.ax_ch3 = hAx;
                set(hAx, 'YDir', 'Reverse');
                BpodSystem.ProtocolFigures.phRaster.nCorrectLine_ch3 = line('XData', NaN, 'YData', NaN, 'Parent', hAx(1));
                BpodSystem.ProtocolFigures.phRaster.nextReverseLine_ch3 = line('XData', NaN, 'YData', NaN, 'Parent', hAx(1), 'Color', 'm');        
            else
                BpodSystem.ProtocolFigures.phRaster.fig_ch3 = [];
                BpodSystem.ProtocolFigures.phRaster.nCorrectLine_ch3 = [];
                BpodSystem.ProtocolFigures.phRaster.nextReverseLine_ch3 = [];
                BpodSystem.ProtocolFigures.phRaster.ax_ch3 = [];
            end
        ...
        case 'update'
            ...
            SoundsToPlot = BpodSystem.ProtocolFigures.phRaster.SoundsToPlot;
            phRStamp = BpodSystem.ProtocolFigures.phRaster.phRStamp;
            lookupFactor = BpodSystem.ProtocolFigures.phRaster.lookupFactor;
            for i = 1:length(SoundsToPlot)
                thisSoundIndex = SoundsToPlot(i);
                outcome_left = onlineFilterTrials_v2('SoundValveIndex', thisSoundIndex,'TrialOutcome', [-1, 0]); % miss or false alarm            
                outcome_right = onlineFilterTrials_v2('SoundValveIndex', thisSoundIndex,'TrialOutcome', [1, 2]); % hit or correct rejection
                ...
                if any(channelsOn == 3)
                    channelData = BpodSystem.PluginObjects.Photometry.trialDFF{3};
                    nSamples = size(channelData, 2);
                    if i == 1
                        set(BpodSystem.ProtocolFigures.phRaster.nCorrectLine_ch3, 'YData', 1:nTrials, 'XData', BpodSystem.Data.SwitchParameter);
                        set(BpodSystem.ProtocolFigures.phRaster.nextReverseLine_ch3, 'YData', 1:nTrials, 'XData', repmat(ls.switchParameterCriterion, 1, nTrials));            
                        if ~isnan(ls.switchParameterCriterion)
                            set(BpodSystem.ProtocolFigures.phRaster.ax_ch3(1), 'YLim', [0 nTrials], 'XLim', [0 max(BpodSystem.Data.SwitchParameter(end) + 1, ls.switchParameterCriterion + 1)]);
                        else
                            if ~isnan(BpodSystem.Data.SwitchParameter(end))
                                set(BpodSystem.ProtocolFigures.phRaster.ax_ch3(1), 'YLim', [0 nTrials], 'XLim', [0 BpodSystem.Data.SwitchParameter(end) + 0.1]);
                            end
                        end
                    end
                    phMean = mean(mean(channelData(:,x1:x2)));
                    phStd = mean(std(channelData(:,x1:x2)));    
                    ax = BpodSystem.ProtocolFigures.phRaster.ax_ch3(i + 1); % phRaster axes start at i + 1

function phDemodOnline2(currentTrial)
...
for counter = 1:length(nidaq.channelsOn)
    ...
    if  numel(nidaq.channelsOnOut)==1
        chOut = 1;
    else
        chOut = nidaq.channelsOnOut(chO);
    end
    ...
    %         end
    nidaq.online.currentDemodData{ch} = phDemod(nidaq.ai_data(:,counter), nidaq.ao_data(:,chOut), nidaq.sample_rate, mod_freq(chOut), lowCutoff);
